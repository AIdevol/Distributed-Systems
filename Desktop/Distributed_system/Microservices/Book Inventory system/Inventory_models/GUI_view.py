...
class SignInPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        def create(self, UserEmail, Password, controller):
            existing_user = session.query(User).filter_by(email=UserEmail.get()).first()

            if existing_user:
                messagebox.showinfo("Info", "User already exists. Please choose a different email.")
            else:
                # Create a new user
                new_user = User(email=UserEmail.get(), password=Password)
